...
impl<'a> RssFeedRepository<'a> {
    ...
    pub async fn list_with_unread(&self, user_id: Option<i64>) -> Result<Vec<RssFeedWithUnread>> {
        ...
        let rows = sqlx::query_as::<_, RssFeedWithUnreadRow>(
            r#"
                   ...
                   (SELECT COUNT(*) FROM rss_items i
                    ...
                    AND i.id > COALESCE(
                        (SELECT last_read_item_id FROM rss_read_positions
                         WHERE user_id = ? AND feed_id = f.id),
                        0)) as unread_count
            FROM rss_feeds f
            WHERE f.is_active = 1 AND f.created_by = ?
            ORDER BY f.id ASC
            "#,
        )
        .bind(user_id)
    ...
    pub async fn update_last_fetched(&self, id: i64) -> Result<bool> {
        let result = sqlx::query(
            "UPDATE rss_feeds SET last_fetched_at = datetime('now'), updated_at = datetime('now') WHERE id = ?",
        )
        .bind(id)
        .execute(self.pool)
        .await
    ...
    pub async fn update_last_item_at(&self, id: i64, last_item_at: DateTime<Utc>) -> Result<bool> {
        let result = sqlx::query(
            "UPDATE rss_feeds SET last_item_at = ?, updated_at = datetime('now') WHERE id = ?",
        )
...
impl<'a> RssItemRepository<'a> {
    ...
    pub async fn count_by_feed(&self, feed_id: i64) -> Result<i64> {
        let count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM rss_items WHERE feed_id = ?")
            .bind(feed_id)
            .fetch_one(self.pool)
            .await
            .map_err(|e| HobbsError::Database(e.to_string()))?;
    ...
    pub async fn delete_by_feed(&self, feed_id: i64) -> Result<u64> {
        let result = sqlx::query("DELETE FROM rss_items WHERE feed_id = ?")
            .bind(feed_id)
            .execute(self.pool)
            .await
            .map_err(|e| HobbsError::Database(e.to_string()))?;
...
impl<'a> RssReadPositionRepository<'a> {
    ...
    pub async fn delete(&self, user_id: i64, feed_id: i64) -> Result<bool> {
        let result =
            sqlx::query("DELETE FROM rss_read_positions WHERE user_id = ? AND feed_id = ?")
                .bind(user_id)
                .bind(feed_id)
                .execute(self.pool)
                .await
    ...
    pub async fn delete_by_user(&self, user_id: i64) -> Result<u64> {
        let result = sqlx::query("DELETE FROM rss_read_positions WHERE user_id = ?")
            .bind(user_id)
            .execute(self.pool)
            .await
            .map_err(|e| HobbsError::Database(e.to_string()))?;
