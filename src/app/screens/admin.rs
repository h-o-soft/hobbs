...
impl AdminScreen {
    ...
    pub async fn run(ctx: &mut ScreenContext, session: &mut TelnetSession) -> Result<ScreenResult> {
        ...
        loop {
            ...
            ctx.send_line(
                session,
                &format!("=== {} ===", ctx.i18n.t("admin.rss_management")),
            )
            .await?;
            ctx.send_line(
                session,
                &format!("  [20] {}", ctx.i18n.t("admin.rss_feed_list")),
            )
            .await?;
            ctx.send_line(
                session,
                &format!("  [21] {}", ctx.i18n.t("admin.rss_add_feed")),
            )
            .await?;
            ctx.send_line(
                session,
                &format!("  [22] {}", ctx.i18n.t("admin.rss_edit_feed")),
            )
            .await?;
            ctx.send_line(
                session,
                &format!("  [23] {}", ctx.i18n.t("admin.rss_delete_feed")),
            )
            .await?;
            ctx.send_line(
                session,
                &format!("  [24] {}", ctx.i18n.t("admin.rss_refresh_feeds")),
            )
            .await?;
            ctx.send_line(session, "").await?;
            ...
            .await?;
            ctx.send_line(session, "  [25] System Status").await?;
            ctx.send_line(session, "").await?;
            ...
            match input.to_ascii_lowercase().as_str() {
                ...
                "20" => Self::show_rss_feeds(ctx, session).await?,
                "21" => Self::add_rss_feed(ctx, session).await?,
                "22" => Self::edit_rss_feed(ctx, session).await?,
                "23" => Self::delete_rss_feed(ctx, session).await?,
                "24" => Self::refresh_rss_feeds(ctx, session).await?,
                "25" => Self::show_system_status(ctx, session).await?,
                _ => {}
            }
        }
    }
    ...
    /// Show RSS feed list for admin.
    async fn show_rss_feeds(ctx: &mut ScreenContext, session: &mut TelnetSession) -> Result<()> {
        use crate::rss::RssFeedRepository;

        loop {
            let feeds = RssFeedRepository::list_all(ctx.db.conn())?;

            ctx.send_line(session, "").await?;
            ctx.send_line(
                session,
                &format!("=== {} ===", ctx.i18n.t("admin.rss_feed_list")),
            )
            .await?;
            ctx.send_line(session, "").await?;

            if feeds.is_empty() {
                ctx.send_line(session, ctx.i18n.t("admin.rss_no_feeds"))
                    .await?;
            } else {
                // Header
                ctx.send_line(
                    ...
                    &format!(
                        "  {:<4} {:<30} {:<8} {}",
                        "No.",
                        ctx.i18n.t("admin.rss_feed_title"),
                        ctx.i18n.t("rss.unread"),
                        ctx.i18n.t("admin.rss_feed_active")
                    ),
                )
                .await?;
                ...
                for (i, feed) in feeds.iter().enumerate() {
                    ...
                    let status = if feed.is_active {
                        ctx.i18n.t("admin.rss_feed_active")
                    } else {
                        ctx.i18n.t("admin.rss_feed_inactive")
                    };

                    let error_info = if feed.error_count > 0 {
                        format!(" (err:{})", feed.error_count)
                    } else {
                        String::new()
                    };
            ...
            let input = ctx.read_line(session).await?;
            if input.trim().eq_ignore_ascii_case("q") || input.trim().is_empty() {
                break;
            }
        }
    ...
    /// Add a new RSS feed.
    async fn add_rss_feed(ctx: &mut ScreenContext, session: &mut TelnetSession) -> Result<()> {
        use crate::db::{Role, UserRepository};
        use crate::rss::{fetch_feed, validate_url, NewRssFeed, RssFeedRepository};

        // Check permission (SubOp or higher)
        let user_id = match session.user_id() {
            Some(id) => id,
            None => {
                ctx.send_line(session, ctx.i18n.t("menu.login_required"))
        ...
        ctx.send_line(
            session,
            &format!("=== {} ===", ctx.i18n.t("admin.rss_add_feed")),
        )
        .await?;
        ctx.send_line(session, "").await?;
        ...
        if let Err(e) = validate_url(url) {
            ctx.send_line(
                session,
                &ctx.i18n
                    .t_with("admin.rss_fetch_error", &[("error", &e.to_string())]),
            )
            .await?;
            return Ok(());
        }
        ...
        ctx.send_line(session, ctx.i18n.t("admin.rss_refreshing"))
            .await?;
        ...
        match fetch_feed(url).await {
            Ok(parsed) => {
                ...
                if let Some(site) = &parsed.site_url {
                    new_feed = new_feed.with_site_url(site);
                }
                ...
                match RssFeedRepository::create(ctx.db.conn(), &new_feed) {
                    Ok(feed) => {
                        ctx.send_line(
                            session,
                            &ctx.i18n
                                .t_with("admin.rss_feed_created", &[("title", &feed.title)]),
                        )
                        .await?;
                    }
                    Err(e) => {
    ...
    /// Edit an RSS feed.
    async fn edit_rss_feed(ctx: &mut ScreenContext, session: &mut TelnetSession) -> Result<()> {
        use crate::db::{Role, UserRepository};
        use crate::rss::{RssFeedRepository, RssFeedUpdate};
        ...
        ctx.send(
            session,
            &format!("{}: ", ctx.i18n.t("admin.rss_feed_number_to_edit")),
        )
        .await?;
        let num_input = ctx.read_line(session).await?;
        ...
        let feed_idx: usize = match num_input.parse::<usize>() {
            Ok(n) if n > 0 && n <= feeds.len() => n - 1,
            _ => {
                ctx.send_line(session, ctx.i18n.t("common.invalid_input"))
                    .await?;
                return Ok(());
        ...
        if feed.error_count > 0 {
            ...
            if let Some(err) = &feed.last_error {
                ctx.send_line(
                    session,
                    &format!("{}: {}", ctx.i18n.t("admin.rss_last_error"), err),
                )
                .await?;
            }
        }
        ...
        match edit_input {
            "1" => {
                // Toggle active
                let update = if feed.is_active {
                    RssFeedUpdate::new().disable()
                } else {
                    RssFeedUpdate::new().enable()
                };
                if RssFeedRepository::update(ctx.db.conn(), feed.id, &update).is_ok() {
                    ctx.send_line(
                        session,
                        &ctx.i18n
                            .t_with("admin.rss_feed_updated", &[("title", &feed.title)]),
                    )
                    .await?;
                }
            }
    ...
    /// Delete an RSS feed.
    async fn delete_rss_feed(ctx: &mut ScreenContext, session: &mut TelnetSession) -> Result<()> {
        use crate::db::{Role, UserRepository};
        use crate::rss::RssFeedRepository;

        // Check permission (SysOp only for delete)
        let user_id = match session.user_id() {
            Some(id) => id,
            None => {
                ctx.send_line(session, ctx.i18n.t("menu.login_required"))
        ...
        ctx.send_line(
            session,
            &format!("=== {} ===", ctx.i18n.t("admin.rss_delete_feed")),
        )
        .await?;
        ctx.send_line(session, "").await?;
        ...
        ctx.send(
            session,
            &format!("{}: ", ctx.i18n.t("admin.rss_feed_number_to_delete")),
        )
        .await?;
        let num_input = ctx.read_line(session).await?;
        let num_input = num_input.trim();
        ...
        ctx.send(
            session,
            &ctx.i18n
                .t_with("admin.rss_confirm_delete", &[("title", &feed.title)]),
        )
        .await?;
        let confirm = ctx.read_line(session).await?;

        if confirm.trim().eq_ignore_ascii_case("y") {
            if RssFeedRepository::delete(ctx.db.conn(), feed.id).is_ok() {
                ctx.send_line(
                    session,
                    &ctx.i18n
                        .t_with("admin.rss_feed_deleted", &[("title", &feed.title)]),
                )
                .await?;
            }
        }
    ...
    async fn refresh_rss_feeds(ctx: &mut ScreenContext, session: &mut TelnetSession) -> Result<()> {
        use crate::db::{Role, UserRepository};
        use crate::rss::{fetch_feed, NewRssItem, RssFeedRepository, RssItemRepository};
        ...
        let feeds = RssFeedRepository::list_active(ctx.db.conn())?;
        let mut success_count = 0;
        let mut error_count = 0;

        for feed in &feeds {
            ctx.send(session, &format!("  {} ... ", feed.title)).await?;
                ...
                Ok(parsed) => {
                    ...
                    for item in &parsed.items {
                        ...
                        if let Some(pub_at) = item.published_at {
                            new_item = new_item.with_published_at(pub_at);
                        }

                        let _ = RssItemRepository::create_or_ignore(ctx.db.conn(), &new_item);
                    }

                    // Update feed - mark as fetched and clear errors
                    let _ = RssFeedRepository::update_last_fetched(ctx.db.conn(), feed.id);
                    let _ = RssFeedRepository::clear_error(ctx.db.conn(), feed.id);

                    ctx.send_line(session, "OK").await?;
                    success_count += 1;
                }
                Err(e) => {
                    // Update error count
                    let _ =
                        RssFeedRepository::increment_error(ctx.db.conn(), feed.id, &e.to_string());
        ...
        ctx.send_line(
            ...
            &format!(
                "{} (OK: {}, Error: {})",
                ctx.i18n.t("admin.rss_refresh_complete"),
                success_count,
                error_count
            ),
        )
