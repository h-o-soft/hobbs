...
impl AdminScreen {
    ...
    pub async fn run(ctx: &mut ScreenContext, session: &mut TelnetSession) -> Result<ScreenResult> {
        ...
        loop {
            ...
            match input.to_ascii_lowercase().as_str() {
                ...
                "20" => Self::show_system_status(ctx, session).await?,
                _ => {}
            }
        }
    }
    ...
    // Note: RSS management functions (show_rss_feeds, add_rss_feed, edit_rss_feed,
    // delete_rss_feed, refresh_rss_feeds) have been removed.
    // RSS is now personal per-user - users manage their own feeds from the RSS screen.
}
