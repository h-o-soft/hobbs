...
impl<'a> MailService<'a> {
    ...
    pub async fn send_mail(&self, request: &SendMailRequest) -> Result<Mail> {
        // Validate subject
        ...
        let mail = mail_repo.create(&new_mail).await?;
    ...
    pub async fn list_sent(&self, user_id: i64) -> Result<Vec<Mail>> {
        let mail_repo = MailRepository::new(self.db.pool());
        let mails = mail_repo.list_sent(user_id).await?;
        Ok(mails)
    }
    ...
    pub async fn get_mail(&self, mail_id: i64, user_id: i64) -> Result<Mail> {
        let mail_repo = MailRepository::new(self.db.pool());
        let mail = mail_repo
            .get_by_id(mail_id)
            .await?
    ...
    pub async fn count_unread(&self, user_id: i64) -> Result<i64> {
        let mail_repo = MailRepository::new(self.db.pool());
        let count = mail_repo.count_unread(user_id).await?;
        Ok(count)
    }
}
...
mod tests {
    ...
    async fn test_send_mail_success() {
        ...
        let mail = service.send_mail(&request).await.unwrap();
    ...
    #[tokio::test]
    async fn test_send_mail_empty_subject() {
        let db = setup_db().await;
        let (sender_id, _) = create_test_users(&db).await;
        let service = MailService::new(&db);

        let request = SendMailRequest::new(sender_id, "bob", "", "Body");
        let result = service.send_mail(&request).await;

        assert!(matches!(result, Err(HobbsError::Validation(_))));
    }

    #[tokio::test]
    async fn test_send_mail_empty_body() {
        let db = setup_db().await;
        let (sender_id, _) = create_test_users(&db).await;
        let service = MailService::new(&db);
    ...
    #[tokio::test]
    async fn test_send_mail_subject_too_long() {
        let db = setup_db().await;
        let (sender_id, _) = create_test_users(&db).await;
        let service = MailService::new(&db);
    ...
    #[tokio::test]
    async fn test_send_mail_recipient_not_found() {
        let db = setup_db().await;
        let (sender_id, _) = create_test_users(&db).await;
        let service = MailService::new(&db);
    ...
    #[tokio::test]
    async fn test_send_mail_to_self() {
        let db = setup_db().await;
        let (sender_id, _) = create_test_users(&db).await;
        let service = MailService::new(&db);
    ...
    #[tokio::test]
    async fn test_send_mail_to_inactive_user() {
        let db = setup_db().await;
        let repo = UserRepository::new(db.pool());
        let user1 = NewUser::new("alice", "password123", "Alice");
        let user2 = NewUser::new("bob", "password123", "Bob");
        let sender_id = repo.create(&user1).await.unwrap().id;
        let bob = repo.create(&user2).await.unwrap();
    ...
    async fn test_list_inbox() {
        ...
        service.send_mail(&request1).await.unwrap();
        service.send_mail(&request2).await.unwrap();

        let inbox = service.list_inbox(recipient_id).await.unwrap();
    ...
    async fn test_list_sent() {
        ...
        service.send_mail(&request).await.unwrap();

        let sent = service.list_sent(sender_id).await.unwrap();
        assert_eq!(sent.len(), 1);
        assert_eq!(sent[0].subject, "Sent Mail");
    }

    #[tokio::test]
    async fn test_get_mail_by_recipient() {
        let db = setup_db().await;
        let (sender_id, recipient_id) = create_test_users(&db).await;
        let service = MailService::new(&db);

        let request = SendMailRequest::new(sender_id, "bob", "Test", "Body");
        let sent_mail = service.send_mail(&request).await.unwrap();
        assert!(!sent_mail.is_read);

        // Recipient views the mail - should be marked as read
        let mail = service.get_mail(sent_mail.id, recipient_id).await.unwrap();
        assert!(mail.is_read);
    }

    #[tokio::test]
    async fn test_get_mail_by_sender() {
        let db = setup_db().await;
        let (sender_id, _) = create_test_users(&db).await;
        let service = MailService::new(&db);
        ...
        let mail = service.get_mail(sent_mail.id, sender_id).await.unwrap();
        assert!(!mail.is_read);
    }

    #[tokio::test]
    async fn test_get_mail_not_found() {
        let db = setup_db().await;
        let (sender_id, _) = create_test_users(&db).await;
        let service = MailService::new(&db);

        let result = service.get_mail(999, sender_id).await;
        assert!(matches!(result, Err(HobbsError::NotFound(_))));
    }

    #[tokio::test]
    async fn test_get_mail_no_permission() {
        let db = setup_db().await;
        let (sender_id, _) = create_test_users(&db).await;
        let repo = UserRepository::new(db.pool());
        let user3 = NewUser::new("charlie", "password123", "Charlie");
        let user3_id = repo.create(&user3).await.unwrap().id;
        ...
        let result = service.get_mail(mail.id, user3_id).await;
        assert!(matches!(result, Err(HobbsError::Permission(_))));
    }

    #[tokio::test]
    async fn test_get_mail_deleted_by_recipient() {
        let db = setup_db().await;
        let (sender_id, recipient_id) = create_test_users(&db).await;
        let service = MailService::new(&db);
        ...
        service.delete_mail(mail.id, recipient_id).await.unwrap();

        // Recipient tries to view - should fail
        let result = service.get_mail(mail.id, recipient_id).await;
        assert!(matches!(result, Err(HobbsError::NotFound(_))));

        // Sender can still view
        let sender_view = service.get_mail(mail.id, sender_id).await;
        assert!(sender_view.is_ok());
    }

    #[tokio::test]
    async fn test_delete_mail_by_sender() {
        let db = setup_db().await;
        let (sender_id, recipient_id) = create_test_users(&db).await;
        let service = MailService::new(&db);
        ...
        service.delete_mail(mail.id, sender_id).await.unwrap();

        // Sender can't see it
        let result = service.get_mail(mail.id, sender_id).await;
        assert!(matches!(result, Err(HobbsError::NotFound(_))));
    ...
    #[tokio::test]
    async fn test_delete_mail_by_both_purges() {
        let db = setup_db().await;
        let (sender_id, recipient_id) = create_test_users(&db).await;
        let service = MailService::new(&db);
        ...
        let db_mail = mail_repo.get_by_id(mail.id).await.unwrap();
        assert!(db_mail.is_none());
    }

    #[tokio::test]
    async fn test_delete_mail_not_found() {
        let db = setup_db().await;
        let (sender_id, _) = create_test_users(&db).await;
        let service = MailService::new(&db);

        let result = service.delete_mail(999, sender_id).await;
        assert!(matches!(result, Err(HobbsError::NotFound(_))));
    }

    #[tokio::test]
    async fn test_delete_mail_no_permission() {
        let db = setup_db().await;
        let (sender_id, _) = create_test_users(&db).await;
        let repo = UserRepository::new(db.pool());
        let user3 = NewUser::new("charlie", "password123", "Charlie");
        ...
        let result = service.delete_mail(mail.id, user3_id).await;
        assert!(matches!(result, Err(HobbsError::Permission(_))));
    }
    ...
    async fn test_count_unread() {
        ...
        assert_eq!(service.count_unread(recipient_id).await.unwrap(), 0);
        ...
        assert_eq!(service.count_unread(recipient_id).await.unwrap(), 2);

        // Read one mail
        service.get_mail(mail2.id, recipient_id).await.unwrap();

        assert_eq!(service.count_unread(recipient_id).await.unwrap(), 1);
    }
}
