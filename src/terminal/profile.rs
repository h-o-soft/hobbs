...
impl TerminalProfile {
    ...
    /// Create a 40-column ShiftJIS terminal profile (40x25, CJK double-width, ANSI enabled).
    ///
    /// This profile is for Japanese 40-column terminals like MSX, PC-6001, etc.
    pub fn col40_sjis() -> Self {
        Self {
            name: "40col_sjis".to_string(),
            width: 40,
            height: 25,
            cjk_width: 2,
            ansi_enabled: true,
    ...
    pub fn col40_utf8() -> Self {
        Self {
            name: "40col_utf8".to_string(),
            width: 40,
            height: 25,
            cjk_width: 2,
            ansi_enabled: true,
...
mod tests {
    ...
    #[test]
    fn test_col40_sjis_profile() {
        let profile = TerminalProfile::col40_sjis();
        assert_eq!(profile.name, "40col_sjis");
        assert_eq!(profile.width, 40);
        assert_eq!(profile.height, 25);
        assert_eq!(profile.cjk_width, 2);
        assert!(profile.ansi_enabled);
