...
| プロトコル | 状態 | 運用推奨 |
|-----------|------|----------|
| Telnet | 平文（レトロ環境互換のため） | VPN/SSHトンネル経由での接続推奨 |
| HTTP API | 平文 | リバースプロキシでHTTPS終端必須 |

- インターネットへの直接公開は非推奨
- 詳細は `docs/operation_guide.md` のセキュリティセクションを参照

#### 認証トークン

- アクセストークン: JWT（署名付き、DB状態に依存しない検証）
- リフレッシュトークン: SHA256ハッシュ化して保存
- ワンタイムトークン: WebSocket/ファイルダウンロード用（30秒有効）
