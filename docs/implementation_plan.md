...
| 10 | 統合・調整 | 画面遷移, E2Eテスト | Phase 1-9 | 進行中 |
...
**完了条件**:
- [ ] ChatScreen から ChatRoomManager への接続
- [ ] チャットルームへの入室処理
- [ ] メッセージ送受信の実装
- [ ] /quit, /who, /me コマンドの動作確認
- [ ] 統合テスト
...
**完了条件**:
- [ ] ScreenContext::new() にエンコーディングパラメータを追加
- [ ] SessionHandler::create_screen_context() で現在のエンコーディングを渡す
- [ ] 入出力のエンコーディング変換を検証
- [ ] 単体テスト・統合テスト
...
**完了条件**:
- [ ] ShiftJIS → 投稿 → ShiftJIS で正常表示
- [ ] ShiftJIS → 投稿 → UTF-8 で正常表示
- [ ] UTF-8 → 投稿 → UTF-8 で正常表示
- [ ] UTF-8 → 投稿 → ShiftJIS で正常表示
- [ ] E2Eテスト追加
