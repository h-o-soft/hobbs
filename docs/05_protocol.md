...
```
            ...
            │                   │                   │
    ┌───────▼───────┐   ┌───────▼───────┐   ┌──────▼──────┐
    │ Telnet Layer  │   │  SSH Layer    │   │ (Future)    │
    │  (Port 2323)  │   │  (Port 2222)  │   │             │
    └───────────────┘   └───────────────┘   └─────────────┘
```

- 現在のアーキテクチャはTelnet固有処理を`src/server/telnet.rs`に局所化済み
- SessionManager/Application層はプロトコル非依存
- SSH対応時は`src/server/ssh.rs`を追加し、共通のセッションインターフェースで接続

**SSH対応で必要な作業（Phase 7以降）：**

1. SSHライブラリの選定（`thrussh`等）
2. `src/server/ssh.rs` の実装
3. SSH用設定項目の追加
4. 統合テスト
